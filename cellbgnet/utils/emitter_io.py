...
from cellbgnet.utils import bookkeeping
...
def get_cellbgnet_meta() -> dict:
    return {
        'version': bookkeeping.cellbgnet_state()
    }
...
def save_h5(path: Union[str, pathlib.Path], data: dict, metadata: dict) -> None:
    ...
    with h5py.File(path, 'w') as f:
        ...
        d = f.create_group('cellbgnet')
        d.attrs.update(get_cellbgnet_meta())
...
def load_h5(path) -> Tuple[dict, dict, dict]:
    ...
    with h5py.File(path, 'r') as h5:
        ...
        meta_cellbgnet = dict(h5['cellbgnet'].attrs)

    return data, meta_data, meta_cellbgnet
...
def load_torch(path) -> Tuple[dict, dict, dict]:
    """
    Loads a torch saved emitterset and returns data, metadata and cellbgnet meta.
    Returns:
        (dict, dict, dict): Tuple of dicts containing
            - **emitter_data** (*dict*): core emitter data
            - **emitter_meta** (*dict*): emitter meta information
    ...
    """
    out = torch.load(path)
    return out['data'], out['meta'], out['cellbgnet']
...
def save_csv(path: (str, pathlib.Path), data: dict, metadata: dict) -> None:
    ...
    cellbgnet_meta_json = json.dumps(get_cellbgnet_meta())
    emitter_meta_json = json.dumps(convert_dict_torch_list(metadata))

    assert "\n" not in cellbgnet_meta_json, "Failed to dump cellbgnet meta string."
...
def load_csv(path: (str, pathlib.Path), mapping: (None, dict) = default_mapping, skiprows: int = 3,
             line_em_meta: Optional[int] = 2, line_cellbgnet_meta: Optional[int] = 1,
    ...
    Args:
        ...
        line_cellbgnet_meta: line ix where cellbgnet metadata is present(set None for no cellbgnet meta)
    ...
    if line_cellbgnet_meta is not None:
        cellbgnet_meta = json.loads(linecache.getline(str(path), line_cellbgnet_meta + 1).strip('# '))
    else:
        cellbgnet_meta = None
    ...
    return data_dict, em_meta, cellbgnet_meta
